...
忘了到哪儿了。还是用0.9.
g要取abs，应该是对的。套到层里面看看。
给raw weight单独写一个这个专用的梯度调整。
往后测。
...
class DigitalMapperFunction_v2_2(torch.autograd.Function):
    ...
    def forward(ctx: Any, *args: Any, **kwargs: Any)->Any:
        input_b_i = args[0]# shape must be [batch, in_features]
        ...
        ctx.save_for_backward(input_b_i, index_of_max_o, out_features_s, raw_weight_shape, input_gt_0_b_i)
        return output
    ...
    def backward(ctx, g_in_b_o):
        ...
        input_gt_0_b_i:torch.Tensor
        
        (input_b_i, index_of_max_o, out_features_s, raw_weight_shape, input_gt_0_b_i) = ctx.saved_tensors
            
        in_features = torch.tensor(input_b_i.shape[1])
        input_needs_grad = torch.tensor([input_b_i.requires_grad], device=input_b_i.device)
            
              #继续，下面一这块。
        #input_reshaped_b_1_i = input_b_i[:,None,:]
        ...
        g_in_abs_b_o_repeati = g_in_abs_b_o.unsqueeze(dim = 2).repeat(1,1,in_features)
        ...
        input_gt_0_b_repeato_i = input_gt_0_b_i.unsqueeze(dim = 1).repeat(1,out_features,1)
        
        NOT_g_x_same_sign_b_o_i:torch.Tensor = g_gt_0_b_o_repeati.logical_xor(input_gt_0_b_repeato_i)
        ...
        grad_for_raw_weight__before_sum__b_o_i:torch.Tensor = g_x_same_sign_b_o_i*g_in_abs_b_o_repeati+g_in_abs_b_o_repeati#还可以乘一个系数
        grad_for_raw_weight_o_i = grad_for_raw_weight__before_sum__b_o_i.sum(dim=0, keepdim=False)
        
        if input_needs_grad.logical_not():
