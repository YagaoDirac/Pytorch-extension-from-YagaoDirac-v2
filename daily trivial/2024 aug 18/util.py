...
def vector_length_norm(input:torch.Tensor, epi = 0.000001)->torch.Tensor:
    ...
    length_of_input_b = input.mul(input).sum(dim=1).sqrt()
    epi_tensor = torch.tensor([epi], device=length_of_input_b.device, dtype=length_of_input_b.dtype)
    length_of_input_safe_b = length_of_input_b.maximum(epi_tensor)
