...
def data_gen_full_adder(bits:int, batch:int):#->Tuple[torch.Tensor, torch.Tensor]:
    ...
    c = torch.randint(0,range,[batch,1])
    target = a+b+c
...
# untested!!!!!
# untested!!!!!
class example_DigitalSignalProcessingUnit_MapperGatesMapper(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, \
        ...
        self.out_mapper_in_features = self.in_mapper_out_features+2
...
# # print(input.shape)
# # print(target.shape)
# model = example_DigitalSignalProcessingUnit_MapperGatesMapper(2,1,1,1,1)
...
model = example_DigitalSignalProcessingUnit_MapperGatesMapper(2,2,1,1,1)
