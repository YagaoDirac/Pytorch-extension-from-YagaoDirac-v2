...
class Binarize_base(torch.nn.Module):
    ...
    __constants__ = ['output_when_ambiguous','big_number',
                     'input_is_01', 'input_range', 
                     'output_is_01', 'output_range']
    output_when_ambiguous: float
    big_number: float
    input_is_01:bool
    input_range:BoundaryPair
    output_is_01: bool
    output_range:BoundaryPair

    def __init__(self, input_is_01:bool, input_range:BoundaryPair,\
        ...
        self.set_big_number(big_number)#sets self.output_range
        #self.update_output_range()#sets self.output_range
    ...
    def update_output_range(self):
        if self.input_is_01:
            if self.output_is_01:
                self.output_range = self.input_range.calc_boundary_01(self.big_number)
            else:#output is np
                self.output_range = self.input_range.calc_boundary_01_to_np(self.big_number)
            pass
        else:#input is real number or np
            if self.output_is_01:
                self.output_range = self.input_range.calc_boundary_non01_to_01(self.big_number)
            else:#output is np
                self.output_range = self.input_range.calc_boundary_np(self.big_number)
            pass
        #end of function
        
    def get_output_range(self)->BoundaryPair:
...
# bp01 = BoundaryPair.from_float(0., 1.)
# '''01 to 01'''
# layer = Binarize_base(True, bp01, 1., True)
# input = torch.tensor([[0., 0.5, 1.]], requires_grad=True)
# print(layer(input), f'should be {layer.get_output_range().lower.item():.4f}, 0.5, {layer.get_output_range().upper.item():.4f}')
# layer.set_big_number(9.)
# print(layer(input), "should be 0.01_, 0.5, 0.99_")

# '''01 to np'''
# layer = Binarize_base(True, bp01, 1., False)
# input = torch.tensor([[0., 0.5, 1.]], requires_grad=True)
# print(layer(input), f'should be {layer.get_output_range().lower.item():.4f}, 0., {layer.get_output_range().upper.item():.4f}')
# layer.set_big_number(9.)
# print(layer(input), "should be -0.99_, 0., 0.99_")

# bpnp = BoundaryPair.from_float(-1., 1.)
# '''non 01 to 01'''
# layer = Binarize_base(False, bpnp, 1., True)
...
# '''non 01 to np'''
# layer = Binarize_base(False, bpnp, 1., False)
# input = torch.tensor([[-1., 0., 1.]], requires_grad=True)
# print(layer(input), f'should be {layer.get_output_range().lower.item():.4f}, 0., {layer.get_output_range().upper.item():.4f}')
# layer.set_big_number(9.)
# print(layer(input), "should be -0.99_, 0., 0.99_")
...
# layer = Binarize_base(False, bpnp, trivial_big_number, True,)
# layer.eval()
...
# layer = Binarize_base(False, bpnp, trivial_big_number, False,)
# layer.eval()
# layer.set_output_when_ambiguous(-1.)
...
# layer = Binarize_base(True, bp01, trivial_big_number, True,)
# layer.eval()
# layer.set_output_when_ambiguous(0.)
# print(layer(input), "should be 0., 0., 1.")
# layer.set_output_when_ambiguous(1.)
...
# layer = Binarize_base(True, bp01, trivial_big_number, False,)
# layer.eval()
# layer.set_output_when_ambiguous(-1.)
# print(layer(input), "should be -1., -1., 1.")
# layer.set_output_when_ambiguous(0.)
...
class Binarize_np_to_01(torch.nn.Module):
    ...
    def __init__(self, input_range:BoundaryPair, big_number = 5., \
        ...
        #this layer also needs the info for gramo.
        self.base = Binarize_base(False, input_range, big_number, True, 
...
# '''the tests below are mainly about the gradient modification layers.'''
# layer = Binarize_np_to_01(BoundaryPair.from_float(-1., 1.), big_number=1.)
# input = torch.tensor([[0.]], requires_grad=True)
# output = layer(input)
# g_in = torch.tensor([[0.0012]])
# torch.autograd.backward(output, g_in,inputs= input)
...
# layer = Binarize_np_to_01(BoundaryPair.from_float(-1., 1.), big_number=5.)
# input = torch.tensor([[0.]], requires_grad=True)
# output = layer(input)
# g_in = torch.tensor([[0.0012]])
# torch.autograd.backward(output, g_in,inputs= input)
...
# Because I already tested the gramo layer in MIG, I don't plan to redo them here.'''
# layer = layer = Binarize_np_to_01(BoundaryPair.from_float(-1., 1.), big_number=1.)
# input = torch.tensor([[0., 0.]], requires_grad=True)
# output = layer(input)
# g_in = torch.tensor([[0.12, 0.12]])
# torch.autograd.backward(output, g_in,inputs= input)
