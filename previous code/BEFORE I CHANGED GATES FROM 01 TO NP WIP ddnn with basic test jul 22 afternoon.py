...
class Binarize_Forward_only_Function(torch.autograd.Function):
    ...
    def forward(ctx: Any, *args: Any, **kwargs: Any)->Any:
        ...
        x_needs_grad = torch.tensor([x.requires_grad])
        ctx.save_for_backward(x_needs_grad)
        return x

    @staticmethod
    def backward(ctx, g):
        requires_grad:torch.Tensor
        requires_grad,  = ctx.saved_tensors
        if requires_grad:
            return g, None
        else:
            return None, None
...
class DigitalMapper_V2(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, \
        ...
        if auto_print_difference:
            self.set_auto_print_difference_between_epochs()
        # the old way.
...
class AND_01(torch.nn.Module):
    ...
    def __init__(self, input_per_gate:int = 2, \
        ...
        self.Binarize = Binarize.create_analog_to_np()
...
class DSPU(torch.nn.Module):
    ...
    def get_info(self, directly_print:bool=False) -> str:
        ...
        result_str += ", AND2:"
        result_str += str(self.first_layer.and_gates)
        result_str += ", OR2:"
        result_str += str(self.first_layer.or_gates)
        result_str += ", XOR2:"
        result_str += str(self.first_layer.xor_gates)
        if directly_print:
            print(result_str)
        return result_str
...
# model = DSPU(input.shape[1],target.shape[1],1,1,1,1, scaling_ratio_for_gramo_in_mapper=200.)
...
#             print(layer.raw_weight, "before update")
#             optimizer.step()
#             print(layer.raw_weight, "after update")
#             pass    
#         pass    
#     #optimizer.param_groups[0]["lr"] = 0.01
#     optimizer.step()
...
model.set_auto_print_difference_between_epochs(True,2)
#model.set_auto_print_difference_between_epochs(True,5)
loss_function = torch.nn.MSELoss()
optimizer = torch.optim.SGD(model.parameters(), lr=0.00001)

iter_per_print = 111#111
print_count = 333333
for epoch in range(iter_per_print*print_count):
    model.train()
    pred = model(input)
    ...
    if True and "print acc":
        if epoch%iter_per_print == iter_per_print-1:
            with torch.inference_mode():
                pred = model(input)
                print(epoch+1, "    epochs/acc    ",bitwise_acc(pred, target))
                pass
            pass
        pass
    if False and "print weight":
        if epoch%iter_per_print == iter_per_print-3:
            model.set_auto_print_difference_between_epochs()
        if epoch%iter_per_print == iter_per_print-1:
            model.reset_auto_print_difference_between_epochs()
            pass
        pass

with torch.inference_mode():
    pred = model(input)
    bitwise_acc(pred, target, True)
    pass
fds=432
