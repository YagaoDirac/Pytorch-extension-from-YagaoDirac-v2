        ...
        with torch.no_grad():
            ...
            if 1. != self.last_acc:
                if self.skip_hard_clamp>0:
                    ...
                    boundary = self.raw_weight_boundary_for_f32.data.detach().clone()
...
layer = DigitalMapper_V1_1(32,1)
layer.raw_weight.data = torch.ones_like(layer.raw_weight.data)
layer.raw_weight.data[0,:2] = torch.tensor([[1.02,1.01]])
layer.set_shaper_factor(1.0035,True)
for epoch in range(5511):
    every = 100
    layer.set_acc(1.)
...
class dry_stack_test_for_digital_mapper_v1_1(torch.nn.Module):
    ...
    def can_convert_into_eval_only_mode(self)->Tuple[torch.Tensor, torch.Tensor]:
        ...
        for layer in self.mid_layers:
            ...
            pass
        self.last_layer.can_convert_into_eval_only_mode()
        pass
