...
class DigitalMapper_V1_4(torch.nn.Module):
    ...
    def forward(self, input:torch.Tensor)->torch.Tensor:
        ...
        if self.training:
            with torch.no_grad():
                the_max_index_o = self.raw_weight.max(dim=1,keepdim=False).indices
                #the_max_index_o = the_max_index_o.squeeze(dim=1)
                
                #flag_add_true_minus_false = the_max.values.lt(10.)
                
                exp_of_raw_weight_o_i = torch.exp(self.raw_weight)
                ...
                flag_some_rand_o = torch.rand(flag_weight_too_soft_o.shape,device=flag_weight_too_soft_o.device).lt(0.5)
                ...
                temp_o = The_A_prime_o.log()
                #temp = temp.log()
                The_raw_ghost_length_o = temp_o-self.raw_weight[self.out_iota,the_max_index_o]
    ...
    def protect_raw_weight(self):
        with torch.no_grad():
            ...
            self.raw_weight[flag_is_nan] = self.raw_weight_min
...
#                 print(model.can_convert_into_eval_only_mode(), "THIS ONE IS NOT IMPORTANT.model.can_convert_into_eval_only_mode")
...
class dry_stack_test_for_digital_mapper_v1_4(torch.nn.Module):
    def __init__(self, in_features: int, out_features: int, mid_width:int, num_layers:int, \
        ...
        for layer in self.all_layers:
            new_scaling_ratio = layer.gramo_for_raw_weight.scaling_ratio*scaling_ratio_scaled_by
            layer.set_scaling_ratio_for_raw_weight(new_scaling_ratio.item())
            pass
...
'''b 50000,i30,o15,m50,l30   ghost_weight_p = 0.0001()/0.1()/0.9()/0.9999()  scaling_ratio_scaled_by = 1   '''
继续
