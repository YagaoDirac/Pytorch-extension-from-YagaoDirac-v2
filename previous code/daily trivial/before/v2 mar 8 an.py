...
class GradientModificationFunction(torch.autograd.Function):
    ...
    def forward(ctx: Any, *args: Any, **kwargs: Any)->Any:
        ...
        #               suppression_factor = torch.tensor([1e-3]))->torch.Tensor:
        #but python grammar punched me.
        x = args[0]
        ...
        # suppression_factor = torch.tensor([0.001]) 
...
scaling_ratio = torch.tensor([1.])
epi=torch.tensor([1e-5])
suppression_factor = torch.tensor([1e-3])
a = torch.tensor([0.], requires_grad=True)
b = GradientModificationFunction.apply(a,scaling_ratio,epi,suppression_factor)
g = torch.autograd.grad(b, a, retain_graph= True)
g_in = torch.tensor([0.00000123])
...
class GradientModification(torch.nn.Module):
    ...
    def __init__(self, scaling_ratio:float = 1., \
        ...
        self.suppression_factor = torch.tensor([suppression_factor], requires_grad=False)
        pass
    def forward(self, x:torch.Tensor)->torch.Tensor:
        ...
        #suppression_factor:torch.Tensor)->torch.Tensor:
...
model = GradientModification(epi=0.01, suppression_factor=0.1)
...
for epoch in range(1):
    model.train()
    pred = model(input)
    loss = loss_function(pred, torch.tensor([0.0000002], requires_grad=True))
    optimizer.zero_grad()
    loss.backward()
    #optimizer.param_groups[0]["lr"] = 0.01
    print(input.grad, "should be 0.002")

    optimizer.step()
    print(input, "should be 0.002")
