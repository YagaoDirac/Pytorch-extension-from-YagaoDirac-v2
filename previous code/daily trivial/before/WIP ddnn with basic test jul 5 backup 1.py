...
def data_gen_half_adder_1bit(batch:int):#->Tuple[torch.Tensor, torch.Tensor]:
...
def data_gen_full_adder(bits:int, batch:int):#->Tuple[torch.Tensor, torch.Tensor]:
    range = 2**bits
    #print(range)
...
class DigitalMapper(torch.nn.Module):
    ...
    def forward(self, input:torch.Tensor)->torch.Tensor:
        ...
        # if not self.training:
        #     print(x.shape, "2 x before")
        #     fds=432
        x = self.inner_raw(x)
        # if not self.training:
        #     print(x.shape, "2 x after")
        #     fds=432
    ...
    def can_convert_into_eval_only_mode(self):
...
class DigitalSignalProcessingUnit_layer(torch.nn.Module):
    ...
    def __init__(self, in_features: int, \
        ...
        self.in_mapper.set_scaling_ratio(100000.)
        
        output_mode = 1#1_is_both
        self.and1 = AND_01(2, output_mode_0_is_self_only__1_is_both__2_is_opposite_only=output_mode)
        self.or1 = OR_01(2, output_mode_0_is_self_only__1_is_both__2_is_opposite_only=output_mode)
        self.xor1 = XOR_01(2, output_mode_0_is_self_only__1_is_both__2_is_opposite_only=output_mode)
    ...
    def forward(self, input:torch.Tensor)->torch.Tensor:
        ...
        # if not self.training:
        #     print(x.shape, "3 x before")
        #     fds=432
        x = self.in_mapper(x)
        # if not self.training:
        #     print(x.shape, "3 x after")
        #     fds=432
            
        pos_from = 0
        pos_to = self.and_gates*2
        and_head = self.and1(x[:, pos_from:pos_to])
        pos_from = pos_to
        pos_to += self.or_gates*2
        or_head = self.or1(x[:, pos_from:pos_to])              
        pos_from = pos_to
        pos_to += self.xor_gates*2
        xor_head = self.xor1(x[:, pos_from:pos_to])
...
# layer.get_info(True)
# fds=432
# '''No dedicated test for this layer. It's tested below in 2 examples.'''

# class DSPU_10(torch.nn.Module):
#     r'''10 DSPU layers in a row. Interface is the same as the single layer version.
    
#     It also needs an extra mapper layer after it. 
...
#         factory_kwargs = {'device': device, 'dtype': dtype}
#         super().__init__()
#         self.DSPU1 = DigitalSignalProcessingUnit_layer(in_features, and_gates, or_gates, xor_gates)
#         dspu_out = self.DSPU1.get_out_features()
#         self.DSPU2 = DigitalSignalProcessingUnit_layer(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU3 = DigitalSignalProcessingUnit_layer(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU4 = DigitalSignalProcessingUnit_layer(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU5 = DigitalSignalProcessingUnit_layer(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU6 = DigitalSignalProcessingUnit_layer(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU7 = DigitalSignalProcessingUnit_layer(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU8 = DigitalSignalProcessingUnit_layer(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU9 = DigitalSignalProcessingUnit_layer(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU10 = DigitalSignalProcessingUnit_layer(dspu_out, and_gates, or_gates, xor_gates)

#         # self.DSPU1.get_info(True)
#         # self.DSPU2.get_info(True)
#         # self.DSPU9.get_info(True)
#         # self.DSPU10.get_info(True)
#         # fds=432
#         #end of function        
...
#     def get_out_feature(self)->int:
#         return self.DSPU10.get_out_features()
...
#     def get_info(self, directly_print:bool=False) -> str:
#         result = f'10 DSPU in a row. In features:{self.DSPU1.in_mapper.in_features}, out features:{self.DSPU10.get_out_features()}, AND2:{self.DSPU1.and_gates}, OR2:{self.DSPU1.or_gates}, XOR2:{self.DSPU1.xor_gates}'
#         if directly_print:
#             print(result)
#         return result
    
#     pass
# # layer = DSPU_10(11,3,4,5)
# # layer.get_info(True)
# # fds=432

# class DSPU_100(torch.nn.Module):
#     r'''100 DSPU layers in a row. Interface is the same as the single layer version.
...
#         factory_kwargs = {'device': device, 'dtype': dtype}
#         super().__init__()
#         self.DSPU10 = DSPU_10(in_features, and_gates, or_gates, xor_gates)
#         dspu_out = self.DSPU10.get_out_feature()
#         self.DSPU20 = DSPU_10(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU30 = DSPU_10(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU40 = DSPU_10(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU50 = DSPU_10(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU60 = DSPU_10(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU70 = DSPU_10(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU80 = DSPU_10(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU90 = DSPU_10(dspu_out, and_gates, or_gates, xor_gates)
#         self.DSPU100 = DSPU_10(dspu_out, and_gates, or_gates, xor_gates)

#         # self.DSPU10.get_info(True)
#         # self.DSPU20.get_info(True)
#         # self.DSPU90.get_info(True)
#         # self.DSPU100.get_info(True)
#         # fds=432
#         #end of function        
...
#     def get_out_feature(self)->int:
#         return self.DSPU100.get_out_feature()
...
#     def get_info(self, directly_print:bool=False) -> str:
#         result = f'100 DSPU in a row. In features:{self.DSPU10.DSPU1.in_mapper.in_features}, out features:{self.get_out_feature()}, AND2:{self.DSPU10.DSPU1.and_gates}, OR2:{self.DSPU10.DSPU1.or_gates}, XOR2:{self.DSPU10.DSPU1.xor_gates}'
#         if directly_print:
#             print(result)
#         return result
    
#     pass
# layer = DSPU_100(11,3,4,5)
# layer.get_info(True)
# fds=432
...
class DSPU(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features:int, \
        ...
        self.first_layer = DigitalSignalProcessingUnit_layer(in_features, and_gates, or_gates, xor_gates)
        ...
        self.out_mapper.set_scaling_ratio(100000.)
        #end of function        
...
class example_1_layer_of_DSPU(torch.nn.Module):
    r'''Notice the mapper after gates.
    ...
    def __init__(self, in_features: int, out_features: int, \
        ...
        factory_kwargs = {'device': device, 'dtype': dtype}
        super().__init__()
        self.DSPU = DSPU(in_features, out_features, and_gates, or_gates, xor_gates, 1)
        #end of function        
    def forward(self, input:torch.Tensor)->torch.Tensor:
        '''This example only shows the training path.'''
        x = input
...
# # print(input.shape)
# # print(target.shape)
# model = example_1_layer_of_DSPU(2,1,1,1,1)
# # model.in_mapper.set_scaling_ratio(100000.)
# # model.out_mapper.set_scaling_ratio(100000.)
...
# batch = 10000
# (input, target) = data_gen_half_adder_1bit(batch)
# model = example_1_layer_of_DSPU(input.shape[1],target.shape[1],1,1,1)
...
class example_4_layers_of_DSPU(torch.nn.Module):
    r'''The test for the 1 bit full adder.
    '''
    #__constants__ = ['',]
    ...
    def __init__(self, in_features: int, out_features: int, \
        ...
        self.out_mapper_out_features = out_features
        
        self.out_mapper = DigitalMapper(dspu_out, self.out_mapper_out_features)
        self.out_mapper.set_scaling_ratio(100000.)
        #end of function        
    ...
    def forward(self, input:torch.Tensor)->torch.Tensor:
        ...
        # if not self.training:
        #     print(x.shape, "4 x before")
        #     fds=432
        x = self.DSPU1(x)
        # if not self.training:
        #     print(x.shape, "4 x after")
        #     fds=432
...
# # print(input)
# # print(output)
# model = example_4_layers_of_DSPU(input.shape[1],target.shape[1],4,4,4)
...
# batch = 1000
# (input, target) = data_gen_full_adder(1,batch)
# model = example_4_layers_of_DSPU(input.shape[1],target.shape[1],4,4,4)
...
# def test_with_input(in_a:int, in_b:int, in_c:int):
...
# test_with_input(0,0,0)
# test_with_input(0,1,0)
# test_with_input(1,0,0)
# test_with_input(1,1,0)
# test_with_input(0,0,1)
# test_with_input(0,1,1)
# test_with_input(1,0,1)
# test_with_input(1,1,1)
