...
out_features_iota = torch.linspace(0, out_features-1, out_features, dtype=torch.int32)
...
input_reshaped = x.unsqueeze(dim=1)
print(input_reshaped, "input_reshaped")
print(input_reshaped.shape, "input_reshaped.shape")
g_in_reshaped = g_in.unsqueeze(dim=-1)
print(g_in_reshaped, "g_in_reshaped")
print(g_in_reshaped.shape, "g_in_reshaped.shape")
grad_for_raw_weight = g_in_reshaped.matmul(input_reshaped)
print(grad_for_raw_weight, "temp_grad_for_raw_weight before sum")
grad_for_raw_weight = grad_for_raw_weight.sum(dim=0, keepdim=True)
grad_for_raw_weight = grad_for_raw_weight.squeeze(dim=0)
print(grad_for_raw_weight, "temp_grad_for_raw_weight after sum")
print(grad_for_raw_weight.shape, "temp_grad_for_raw_weight.shape")
print(raw_weight.shape, "raw_weight.shape")# these 2 should be the same.
...
the_heaviest_tensor = torch.zeros_like(raw_weight).repeat(x.shape[0],1, 1)
print(the_heaviest_tensor.shape, "the_heaviest_tensor.shape")
the_heaviest_tensor[:, out_features_iota, index_of_max] = 1.#g_in
print(the_heaviest_tensor, "the_heaviest_tensor")

g_in_reshaped_repeated = g_in_reshaped.repeat(1, 1, in_features)
print(g_in_reshaped_repeated.shape, "g_in_reshaped_repeated.shape")

the_heaviest_tensor = the_heaviest_tensor.mul(g_in_reshaped_repeated)
print(the_heaviest_tensor, "the_heaviest_tensor")
...
grad_for_x = the_heaviest_tensor.sum(dim=1, keepdim=True)
grad_for_x = grad_for_x.squeeze(dim=1)
print(grad_for_x, "grad_for_x")
print(grad_for_x.shape, "grad_for_x.shape")
...
__proxy_weight.data[out_features_iota, index_of_max] = 1.
...
class DigitalMapper_V2(torch.autograd.Function):
    ...
    def forward(ctx: Any, *args: Any, **kwargs: Any)->Any:
        ...
        # the test code. x = torch.tensor([[5., 6., 7.], [8., 9., 13]])
        in_features = x.shape[1]
        ...
        #print(grad_for_raw_weight, "temp_grad_for_raw_weight after sum")
        #print(grad_for_raw_weight.shape, "temp_grad_for_raw_weight.shape")
        #print(raw_weight.shape, "raw_weight.shape")
    ...
    def backward(ctx, g_in):
        ...
        x  ,  a,  index_of_max = ctx.saved_tensors
        ...
        #test code.  g_in = torch.tensor([[0.30013, 0.30103, 0.30113, 0.31003, 0.31013], [0.40013, 0.40103, 0.40113, 0.41003, 0.41013], ])
        #print(output.shape, "output.shape, should be ", g_in.shape)

        input_reshaped = x.unsqueeze(dim=1)
        print(input_reshaped, "input_reshaped")
        print(input_reshaped.shape, "input_reshaped.shape")
        g_in_reshaped:torch.Tensor = g_in.unsqueeze(dim=-1)
        #print(g_in_reshaped, "g_in_reshaped")
        #print(g_in_reshaped.shape, "g_in_reshaped.shape")
        grad_for_raw_weight = g_in_reshaped.matmul(input_reshaped)
        print(grad_for_raw_weight, "temp_grad_for_raw_weight before sum")
        ...
        '''x = args[0] raw_weight = args[1]'''
        return grad_for_x, grad_for_raw_weight
