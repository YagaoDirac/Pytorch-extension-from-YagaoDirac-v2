...
class DSPU(torch.nn.Module):
    ...
    def set_scaling_ratio_for_first_layer(self, scaling_ratio:float):
        '''simply sets the inner layer.'''
        self.first_layer.set_scaling_ratio_for_inner_raw(scaling_ratio)
        pass
    def set_scaling_ratio_for_middle_layers(self, scaling_ratio:float):
        '''simply sets the inner layer.'''
        for layer in self.second_to_last_layers:
            layer.set_scaling_ratio_for_inner_raw(scaling_ratio)
            pass
        pass
    def set_scaling_ratio_for_out_mapper(self, scaling_ratio:float):
        '''simply sets the inner layer.'''
...
on going:
3 bit, 15 layer 32 x3(and, or, xor), batch 500, lr 0.00001///epoch ???, acc ???
'''
准备
准备
准备
...
layer = 15
gates = 32
batch = 500     # doesn't affect the accumulated grad.
...
for epoch in range(iter_per_print*print_count):
    ...
    if False and "compares the weight and grad":
        if epoch%iter_per_print == iter_per_print -1:
            ...
            pass
        pass
    if False and "weight before/after step":
        layer = model.first_layer
        print(layer.get_mapper_raw_weight()[:2][:16], "first_layer raw weight")
        optimizer.step()
        print(layer.get_mapper_raw_weight()[:2][:16], "first_layer raw weight")
        layer = model.second_to_last_layers[0]
        print(layer.get_mapper_raw_weight()[0][:16], "2nd_layer raw weight")
        optimizer.step()
        print(layer.get_mapper_raw_weight()[0][:16], "2nd_layer raw weight")
        ...
        print(layer.inner_raw.raw_weight[0][:16], "out_mapper raw weight")
        optimizer.step()
        print(layer.inner_raw.raw_weight[0][:16], "out_mapper raw weight")
        fds=432
        pass  
