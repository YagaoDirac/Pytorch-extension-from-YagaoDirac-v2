...
# to do list: test this one further. I don't usually use sigmoid. Maybe I messed with some detail?
...
# loss_function = torch.nn.MSELoss()
# data_amount = 100
# input = torch.rand([data_amount, in_feature])+1
# target = torch.rand([data_amount, 1])+1
# # print(input)
# # print(target)
# optimizer = torch.optim.SGD(model.parameters(), lr=0.1)
...
# More data sometimes helps, but according to my test, gramo never helps with muted neutrons. 
# Maybe I should go back to sigmoid?
# Or maybe I should make the model wider????
# '''
...
# data_amount = 100
# input = torch.rand([data_amount, in_feature])+1
# target = torch.rand([data_amount, 1])+3
# # print(input)
# # print(target)
...
#         print(model.linear2.weight.grad[0:3])
#         print(model.linear3.weight.grad[0:3])
#         print("--------------")
...
When the input, inside with, sample count are all small, it works
But when I scaled it, it stopped working.
'''
...
#         div_me_when_too_small = 0.0001
        
#         self.gramo1  = GradientModification(scaling_ratio=math.pow(self.width_inside, 0.25)/math.sqrt(in_feature),\
#                                              div_me_when_g_too_small=div_me_when_too_small)
#         #ok, I'm giving up with this formula. I should test relu more.
#         scaling_ratio_for_mid = math.pow(self.width_inside, 0.25)/math.sqrt(self.width_inside)
#         self.gramo2  = GradientModification(scaling_ratio=scaling_ratio_for_mid,div_me_when_g_too_small=div_me_when_too_small)
#         self.gramo3  = GradientModification(scaling_ratio=scaling_ratio_for_mid,div_me_when_g_too_small=div_me_when_too_small)
#         self.gramo4  = GradientModification(scaling_ratio=scaling_ratio_for_mid,div_me_when_g_too_small=div_me_when_too_small)
#         self.gramo5  = GradientModification(scaling_ratio=scaling_ratio_for_mid,div_me_when_g_too_small=div_me_when_too_small)
#         self.gramo6  = GradientModification(scaling_ratio=scaling_ratio_for_mid,div_me_when_g_too_small=div_me_when_too_small)
#         self.gramo7  = GradientModification(scaling_ratio=scaling_ratio_for_mid,div_me_when_g_too_small=div_me_when_too_small)
#         self.gramo8  = GradientModification(scaling_ratio=scaling_ratio_for_mid,div_me_when_g_too_small=div_me_when_too_small)
#         self.gramo9  = GradientModification(scaling_ratio=scaling_ratio_for_mid,div_me_when_g_too_small=div_me_when_too_small)
#         self.gramo10 = GradientModification(scaling_ratio=out_features/self.width_inside,div_me_when_g_too_small=div_me_when_too_small)
