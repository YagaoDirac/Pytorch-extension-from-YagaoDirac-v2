...
But the actual a typical value is +- 1.41/(dim)  *  4*(dim-1)/(dim*dim).( or not 1.41, but 2.82???)
When dim is big, the untouched training dynamics is not useful.
...
I tested the "expansion" method to make the vector-length of grad more contrastive. 
But before that, they all divided by the top-length of all vector-length, to make them inside [0, 1].
Then the length become pow(length,some_factor). I tested these factors, -1 to 2(maybe 3?) is best.
To save computation, I decide to use 1, which means no expansion calculation.

Then the length is multiplied with some cap_to, to make them [0, cap_to]. 
The cap_to feels like a learning-rate.

in the part 2( I guess so), I tested 1-step correction or 2-step correction, I scanned the param.
I also tested a bit about the adaptive param-combination.

Then I move to the length correction.

The most useful measurement is the orthogonal_matrix measurement, and the angle_score is the most useful.
'''
...
# I moved anything below here to the part 2.py
