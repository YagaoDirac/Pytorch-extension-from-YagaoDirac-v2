...
class DigitalMapper_Non_standard(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, \
                    ...
                    device=None, dtype=None, debug_info = "") -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super().__init__()
...
class DigitalMapper(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, \
        ...
        self.inner_raw = DigitalMapper_Non_standard(in_features, out_features, \
                            ...
                            device=None, dtype=None, debug_info = debug_info)
    ...
    def forward(self, input:torch.Tensor)->torch.Tensor:
        ...
        #     fds=432
        #fds = self.inner_raw.raw_weight.dtype
        x = self.inner_raw(x)
        # if not self.training:
...
class DigitalSignalProcessingUnit_layer(torch.nn.Module):
    ...
    def forward(self, input:torch.Tensor)->torch.Tensor:
        ...
        #     fds=432
        #fds = self.in_mapper.inner_raw.raw_weight.dtype
        x = self.in_mapper(x)
        # if not self.training:
        ...
        and_head:torch.Tensor = self.and1(x[:, pos_from:pos_to])
        pos_from = pos_to
...
class DSPU(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features:int, \
        ...
        self.first_layer = DigitalSignalProcessingUnit_layer(
            in_features, and_gates, or_gates, xor_gates, debug_info = "first layer")
        
        mid_width = self.first_layer.get_out_features()
        self.second_to_last_layers = torch.nn.modules.container.ModuleList( \
                [DigitalSignalProcessingUnit_layer(mid_width, and_gates, or_gates, xor_gates, debug_info=f'{i}') \
                    for i in range(num_layers-1)])
        self.num_layers = num_layers
           
        self.out_mapper = DigitalMapper(mid_width, out_features, debug_info="out mapper")
...
'''
f16 doesn't train. Super weird.
f32:
1 bit, 4 layer 4 x3(and, or, xor), batch 10, lr 0.00001///epoch 300, acc 100%
2 bit, 7 layer 16 x3, batch 100, lr 0.000001///epoch 3000, acc 100%
3 bit, 10 layer 32 x3, batch 100(a bit too small), lr 0.000001///epoch 5000, acc 100%
3 bit, 10 layer 32 x3, batch 1000, lr 0.000001///epoch 3000, acc 100%
4 bit, 13 layer 64 x3, batch 100(a bit too small), lr 0.000001///epoch 13k, acc 100%
4 bit, 13 layer 128 x3, batch 1000, lr 0.000005///epoch 17k, acc 80~90%
4 bit, 14 layer 64 x3, batch 1000, lr 0.000005?1///1k 0.7 >>> 7k 0.8 >>> 20k 0.6
4 bit, 14 layer 128 x3, batch 1000, lr 0.000005///5k 0.7 >>> 0.6
4 bit, 15 layer 128 x3, batch 1000, lr 0.000005///5k 0.7 >>> 0.6
I add the pre-step noise for grad, and continue testing. It's basically a DropOut.

to do:
5 bit, 13 layer 64,64,64, batch 300, lr 0.000001///
failed:
4 bit, 13 layer 128 x3, batch 5000, lr 0.0000002///xxx

######failed 7bit, 22 layer 512,512,512, batch 1000, epoch ???, lr 0.001, ???%

4 bit, 15 layer 128 x3, batch 1000, lr 0.000005, noise base 1.2///5k 0.6
on going:
4 bit, 15 layer 128 x3, batch 500, lr 0.000002, noise base 1.8///5k 0.7 >>> 0.6
'''
bit = 4
layer = 15
gates = 128
...
for epoch in range(iter_per_print*print_count):
    if epoch%iter_per_print == iter_per_print -1:
        ...
        with torch.inference_mode():
            ...
            temp = pred.eq(target)
            print(temp.sum()/float(temp.shape[0]))
            fds=432
            temp = temp.sum()
            temp = temp.to(torch.float32)
            acc = temp/float(batch*target.shape[1])
