...
class Binarize_base(torch.nn.Module):
    ...
    def get_output_range(self, input_range:BoundaryPair)->BoundaryPair:
        if 0 == self.input_is_0_for_01__1_for_np__2_for_float:
            #input is 01
            if self.output_is_01:
                return input_range.calc_boundary_01(self.big_number)
            else:#output is np
                return input_range.calc_boundary_01_to_np(self.big_number)
            pass
        else:#input is real number or np
            if self.output_is_01:
                return input_range.calc_boundary_non01_to_01(self.big_number)
            else:#output is np
                return input_range.calc_boundary_np(self.big_number)
            pass
        #end of function
