...
class AND_np(torch.nn.Module):
    ...
    def __init__(self, input_per_gate:int = 2, \
        ...
        self.Binarize = Binarize.create_analog_to_np(False)
...
class DigitalSignalProcessingUnit_layer(torch.nn.Module):
    ...
    def forward(self, input:torch.Tensor)->torch.Tensor:
        ...
        if len(self.and_gate.Binarize.gramos)>0:
            raise Exception("self.and_gate has gramo !!!")
        if len(self.or_gate.Binarize.gramos)>0:
...
# If comment the 
# >>> x = self.out_gramo(x)
# out, there's no gramo to protect the grad.
# '''
