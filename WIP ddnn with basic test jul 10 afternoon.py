...
class DigitalMapper(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, \
                    scaling_ratio_for_gramo:float = 10., \
                    auto_merge_duration:int = 20, raw_weight_boundary_for_f32:float = 15., \
                    device=None, dtype=None) -> None:   #, debug_info = ""
        factory_kwargs = {'device': device, 'dtype': dtype}
        super().__init__()
...
class DigitalSignalProcessingUnit_layer(torch.nn.Module):
    ...
    def __init__(self, in_features: int, \
        and_gates: int, or_gates: int, xor_gates: int, \
            scaling_ratio_for_gramo_in_mapper:float = 10., \
                    device=None, dtype=None) -> None:       #, debug_info = ""
        factory_kwargs = {'device': device, 'dtype': dtype}
        super().__init__()
    ...
    def get_gates_big_number(self)->Tuple[float,float,float]:
        big_number_of_and_gate = self.and_gate.Binarize1.Binarize.base.big_number
        big_number_of_or_gate = self.or_gate.Binarize1.Binarize.base.big_number
        big_number_of_xor_gate = self.xor_gate.Binarize1.Binarize.base.big_number
    ...
    def set_gates_big_number(self, big_number_of_and_gate:float,
        ...
        if big_number_of_and_gate>0.:
            self.and_gate.Binarize1.Binarize.base.big_number = big_number_of_and_gate
        if big_number_of_or_gate>0.:
            self.or_gate.Binarize1.Binarize.base.big_number = big_number_of_or_gate
        if big_number_of_xor_gate>0.:
            self.xor_gate.Binarize1.Binarize.base.big_number = big_number_of_xor_gate
        #end of function.
...
class DSPU(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features:int, \
        ...
        self.first_layer = DigitalSignalProcessingUnit_layer(
            ...
            scaling_ratio_for_gramo_in_mapper=scaling_ratio_for_gramo_in_mapper)#, debug_info = "first layer")
        ...
        self.out_mapper = DigitalMapper(mid_width, out_features,
                                       scaling_ratio_for_gramo=scaling_ratio_for_gramo_in_mapper)# , debug_info="out mapper")
        #end of function        
    ...
    def set_scaling_ratio_for_all(self, scaling_ratio:float):
        '''simply sets the inner layer.'''
    ...
    def get_gates_big_number_from_all(self)->List[Tuple[float,float,float]]:
        result:List[Tuple[float,float,float]] = List()
...
layer = 13
gates = 1
batch = 100     # doesn't affect the accumulated grad.
lr = 0.001      #0.001
noise_base = 1.
...
继续
继续
print(model.first_layer.get_mapper_scaling_ratio_for_inner_raw())
print(model.first_layer.get_gates_big_number())
...
for epoch in range(iter_per_print*print_count):
    ...
    if True:
        layer = model.first_layer
        print(layer.get_mapper_raw_weight(), " raw weight")
        pass    
    optimizer.step()
    if True:
        layer = model.first_layer
