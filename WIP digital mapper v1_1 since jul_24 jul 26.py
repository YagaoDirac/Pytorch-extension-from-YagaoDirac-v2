...
class DigitalMapper_V1_1(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, \
        ...
        self.shaper_factor = torch.nn.Parameter(torch.tensor([shaper_factor], requires_grad=False, **factory_kwargs))
        ...
        self.soft_clamp_threshold1 = torch.nn.Parameter(torch.tensor([3.]), requires_grad=False)
        self.soft_clamp_threshold2 = torch.nn.Parameter(torch.tensor([7.]), requires_grad=False)
        self.soft_clamp_margin = torch.nn.Parameter(torch.tensor([1.]), requires_grad=False)
        ...
        self.the_threshold__can_convert_to_eval_only_mode = torch.nn.Parameter(torch.tensor([0.7], requires_grad=False, **factory_kwargs))
    ...
    def set_shaper_factor(self, shaper_factor:float):
        if shaper_factor<=1.:
            raise Exception("Param:shaper_factor must > 1.")
        if shaper_factor>1.2:
    ...
    def set_acc(self, acc:float, protect_param = False):
    ...
    def __protect_raw_weight(self):
        with torch.no_grad():
            ...
            # don't delete this part. The soft clamping replaced this part.
            # # 0 mean.                    
