...
In pytorch 2.12, there exists a feature. When you create a torch.nn.Parameter, the 
creation behavior stores some shape related info in some backend. Idk the detail, but 
probably some backend. If you set the data inside the Parameter object, that stored
info is not updated. So if you use this Parameter later, pytorch raises with some
shape error.

In order to counter this, the simpliest method is to store the Parameter inside 
a torch.nn.ParameterList. Then, every time you need auto completion, you write
torch.nn.Parameter.the_name and then ctrl+space(the short cut in vs code.). 
When you need to set the data inside the Parameter, you do
self.the_param_list[0] = torch.nn.Parameter(torch.tensor([1., 2, 3], dtype = torch.float32))
Keep it in mind, in pytorch, only float point number is allowed to require_grad.

In the part_1, I showed how to refresh the entire torch.nn.Module subclass object to 
solve this problem. In part_2, I showed how to use torch.nn.ParameterList to solve it.

This issue is from some assumption. Similar assumptions are usually made bc the 
way the tool is design. Although I call it a bug, but to be fair, it's not a bug.
I don't believe a lot of you guys need to reshape the data inside a parameter the way
I show in this test. So, if you don't reshape it like this, you don't need this trick.
'''
