...
class Binarize_base(torch.nn.Module):
    ...
    The output can only be the extreme value. For output mode of 01, it's either 0., or 1.. For np it's -1. or 1..
...
#    def reset_parameters(self) -> None:
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if self.training:
            if self.big_number!=1.:
                x = input*self.big_number
            else:
                x = input
                pass
            if self.output_is_01:
                output = self.gramo(torch.sigmoid(input))
            else:
                output = self.gramo(torch.tanh(input))
                pass
            return output
        
        else:#eval mode
            '''I know the bit wise operations are much faster. 
            If this were ONNX, it should have been bit wise. 
            But this layer here in pytorch is not meant to do that.
            The output will be f32.
            '''
    ...
    def extra_repr(self) -> str:
        '''WIP'''
        if self.training:
            return f'In training mode'
        else:
            return "In evaluating mode"
...
笔记，昨天写了上面那个base，
后面要做的，基于这个base，改出6个不同范围的具体层。

然后门，adc/dac，还有一个什么，依次做。这次的代码肯定比上次多。
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if self.training:
            ...
            with torch.inference_mode():
                ...
                if 1. == self.output_when_ambiguous:
                    return input.ge(0.).to(input.dtype)
                flag = input.eq(0.).to(input.dtype)
                head_output_when_ambiguous = (flag*self.output_when_ambiguous).to(input.dtype)
                head_other = (1-flag)*input.gt(0.).to(input.dtype)
                return head_output_when_ambiguous+head_other
        #End of The function
